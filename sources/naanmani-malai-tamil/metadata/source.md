...
- Scan 4: `என்னுரை`.
- Scan 5: `பதிப்புரை`; it explicitly describes the volume as a collection of four short plays and names **பரதாயணம்**, **அனார்கலி**, **சாக்ரடீஸ்**, and **சேரன் செங்குட்டுவன்**.
- Scans 6–17: **பரதாயணம்**.
...
A Gemini first-pass transcription covering the volume was supplied by the user in chat. It is navigation assistance only. Direct scan comparison is mandatory before `verified` status.

The first page-level check already shows that the first pass contains OCR/order errors around speaker labels and dialogue in scan 6, so it must not be imported wholesale.
