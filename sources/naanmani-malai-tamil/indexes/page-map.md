# Physical Page Map — கலைஞரின் நான்மணி மாலை

Printed pagination is recorded only where directly visible.

| Scan page(s) | Printed page(s) | Section / work | Current state | Notes |
|---|---|---|---|---|
| 1–5 | — | shared front matter | partial | Cover/title/rights/`என்னுரை`/`பதிப்புரை`; full front-matter transcription pending. |
| 6–17 | — / 2–12 | `பரதாயணம்` | assembly-reviewed | Tamil page/assembly PASS; English translation PASS. |
| 18–26 | 13–21 | `அனார்கலி` | assembly-reviewed | 9/9 verified; 4/4 scenes assembled; fidelity PASS. |
| 27–28 | 22–23 | `சாக்ரடீஸ்` — introductory note | verified | 2/2 re-reconciled after user old-glyph correction. |
| 29 | 24 | `சாக்ரடீஸ்` — `காட்சி—1` opening | assembly-reviewed | Final Gemini/source reconciliation complete; `காணா`, `புலிநிகர்`; ends mid-Socrates speech at `எனக்கு`; Scene-1 assembly/fidelity PASS. |
| 30 | 25 | `சாக்ரடீஸ்` — `காட்சி—1` continuation | assembly-reviewed | Final Gemini/source reconciliation complete; `மின்னலப்பா`, `கிண்ணாரக்`, `நட்டாற்றில்`, `ஏண்டி`; scan-proven `சுடுகாடு`; Scene-1 assembly/fidelity PASS. |
| 31 | 26 | `சாக்ரடீஸ்` — `காட்சி—1` closing | assembly-reviewed | Final Gemini/source reconciliation complete; `உயிரினுமினியவர்`, source punctuation, water-pouring sequence and printed `*`; Scene-1 assembly/fidelity PASS. |
| 32 | 27 | `சாக்ரடீஸ்` — `காட்சி—2` | assembly-reviewed | Gemini words retained after user-directed lexical rollback (`கவிஞனாம் மெலிடசும்`, `அரசியல் நிபுணனாம் நீயும்`); scan controls heading, punctuation, long dash, label spacing and final `*`; Scene-2 assembly/fidelity PASS. |
| 33 | 28 | `சாக்ரடீஸ்` — `காட்சி—3` | not-started | Structural survey only. |
| 34–39 | 29–34 | `சாக்ரடீஸ்` — `காட்சி—4` | not-started | Structural survey only. |
| 40–43 | 35–38 | `சாக்ரடீஸ்` — `காட்சி—5` | not-started | Structural survey only. |
| 44–53 | 39–48 | `சேரன் செங்குட்டுவன்` | not-started | Four scenes; direct visual verification pending. |
| 54 | — | back cover | partial | Archival description pending. |

## Current checkpoint

`சாக்ரடீஸ்` scans **27–32 = 6/6 pages processed and verified** for the introduction plus Scenes 1–2 page gates. `காட்சி—1` and `காட்சி—2` are both assembled with page-record fidelity **PASS**. Overall work progress: **6/17 verified**; scenes assembled **2/5**.

Next: verify `காட்சி—3`, scan **33** / printed p.28, under the Gemini-words / scan-typography rule; do not assemble it in that same activity.
