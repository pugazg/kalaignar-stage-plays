...
| Work | Status |
|---|---|
| [ஒரே முத்தம்](works/ore-mutham/) | **ACTIVE — 131 scans registered; source processing 15/131; verified 4; needs-review 11; historical-glyph pass 4/131; checksum pending; structure/assembly not started; English not authorized** |
...
- scan **8** / printed p.6: `காட்சி 1.`;
- scan **9** / printed p.7: `காட்சி 2.`;
- scan **11** / printed p.9: `காட்சி 3.`;
- scan **15** / printed p.13: `காட்சி 4.`;
- scan **130** is printed p.128 and the final dramatic page.

Processed scans 1–15:

- **verified:** `2, 3, 4, 6`;
- **needs-review:** `1, 5, 7–15`;
- full historical-glyph passes: `2, 3, 4, 6`;
- scan 6 fully verifies the author foreword, including the Sri Lankan historical-fragment basis and first staging by `தேவி நாடக சபை`;
- source scene structure is directly anchored through `காட்சி 4.`;
- dense small-type holds remain explicit rather than reconstructed.

Current secure historical-glyph references include `லை`, `ணா`, `னை`, and `றா`; see `works/ore-mutham/HISTORICAL_GLYPH_AUDIT.md` for exact source examples.

Current metrics:

- source scans registered: **131 / 131**;
- page/source processing: **15 / 131**;
...
The source scan is controlling authority. User catalog context is kept distinct from transcription. Scan 6 independently supports the Sri Lankan historical basis and Devi Nataka Sabha first-staging statement; the catalog caste-abolition thematic note is not promoted into source transcription without body evidence.

**Exact next activity:** process source scans **16–20** with direct visual fidelity, printed-page confirmation and the full 13-family historical-glyph gate. Preserve current holds (`1, 5, 7–15`) unless enlarged source evidence genuinely resolves them. Compute the PDF SHA-256 when byte-level hashing becomes available; do not infer a value.
...
`காகிதப்பூ` remains fully closed for the defined Tamil archive + first source-faithful English layer: Tamil **41/41**, source-representation scenes **23/23**, English **23/23**, final reviews **PASS**.
