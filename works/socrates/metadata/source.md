...
- Total Tamil page-level verification: **7/17**
- Dramatic-body page verification: **5/15**
...
On scan 33, Gemini supplies lexical wording. The scan controls `காட்சி—3.`, punctuation, speaker-label spacing, the long dash in `அறிவுத் தங்கத்தை — விடு`, physical line evidence and final source marks. Scan 33 contains no closing `*`; assembled `scenes/03.md` preserves that absence.

Next activity: begin `காட்சி—4` page verification at scan **34** / printed p.29 only under the same Gemini-words / scan-typography rule. Do not assemble `காட்சி—4` in the same activity.
