...
- `காட்சி—3` scan **33**: **1/1 verified; assembly pending**
...
On scan 33, Gemini supplies lexical wording. The scan controls `காட்சி—3.`, punctuation, speaker-label spacing, the long dash in `அறிவுத் தங்கத்தை — விடு`, physical line evidence and final source marks. Scan 33 contains no closing `*`.

Next activity: assemble `காட்சி—3` from verified `pages/0033.md` and run its fidelity audit; do not start scan 34 in the same activity.
