...
This is a living register for recurring culturally, historically, politically and rhetorically loaded Tamil terms.

A settled English form here controls later scene translation unless a scene-specific context requires a documented exception.

## Status key

- **locked** — use consistently unless direct contextual evidence requires review;
- **preferred** — default rendering, but context may justify another form;
- **contextual** — no automatic one-to-one English equivalent;
- **pending** — decide when first encountered in a translation scene.
...
| `தமிழர்` | preferred | `Tamil people`, `Tamils` | Choose by syntax/register. Avoid reducing to merely “Tamil speakers” when peoplehood is explicit. |
| `செந்தமிழ்` | contextual | `refined Tamil`, `classical Tamil`, occasionally retain `Senthamizh` in a note | Do not automatically use `pure Tamil`; that may import a later purist nuance not required by every context. |
...
| `மறையவன்` | contextual | descriptive rendering based on context, e.g. `man of the Vedas / learned in the Vedas` | Do not automatically equate with `Brahmin`. |
...
| `எழுச்சி` | contextual | `awakening`, `upsurge`, `rising` | Preserve mobilizing force; do not automatically render as political `uprising`. Scene 1 → `a new awakening`. |
| `அறம்` | contextual | `virtue`, `justice`, `ethical order`, `aram` | Decide from dramatic context; avoid flattening every occurrence to one English word. |
...
Terminology should preserve concepts that later carry Dravidian movement force — especially `தமிழகம்`, `தாயகம்`, `சுயமரியாதை`, language pride, dignity, justice, anti-hierarchical argument and Tamil historical consciousness — **but the English dialogue must not acquire movement vocabulary that is absent from the Tamil source**.

Any movement relation belongs in a separately labelled interpretive note unless the source itself uses the relevant term.
