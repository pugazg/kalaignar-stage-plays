...
**ACTIVE — P0 SOURCE INTAKE PASS; SCANS 1–150 / 150 OF 188 CANONICAL + INITIAL-VERIFIED + H-GATE PASS + FINAL `verified`; BATCHES 01–15 PASS; NEXT SCANS 151–160.**
...
Historical-glyph corrections recorded through scan 150: **31 total**.

Batch 13 added six source-proven historical-glyph correction loci, advancing the cumulative count **23 → 29**. Batch 14 added scan 138 apparent/Pass-A `கண்ண மறைக்கும் பேய்` → **`கண்ணை மறைக்கும் பேய்`** (`ணை`). Batch 15 added:

- scan 146 apparent/Pass-A `இளஞரே` → **`இளைஞரே`** (`ளை`).

The cumulative correction count therefore advances **30 → 31**. No unresolved textual or glyph loci remain in scans 141–150.
...
Scan 130→131 preserves the physical split `புறப்` → `படுகின்றனர்.`. Scan 146→147 preserves the same-utterance physical continuation `ஆரணங்கின்` → `மயக்கத்தால்...`. Scan 150 closes Scene 46 and does not imply a textual continuation into scan 151.
...
- historical-glyph corrections — **31 total**;
...
3. Pass A: process scans **151–160** whole-page once, creating `pages/0151.md` through `0160.md` and establishing only source-proven scene/physical boundaries.
...
7. Create `BATCH_16_REVIEW.md`, synchronize page map, glyph audit, READMEs, handover and next prompt, then fetch final live `main` SHA.
